...
test("V30 overlays use neutral glass while blue stays a small information accent", async () => {
  ...
  assert.match(index, /styles\/home-v30-polish\.css\?v=20260901-32/);
...
test("V30 home owns the desktop navigation chrome without duplicating the global header", async () => {
  const css = await readFile(new URL("../styles/home-v30-polish.css", import.meta.url), "utf8");

  assert.match(css, /body:has\(\.home-journey\)\s+\.site-header\s*,\s*body:has\(\.home-journey\)\s+\.site-footer\s*\{\s*display:\s*none;/s);
});
