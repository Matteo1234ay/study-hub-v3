...
import {createSectionObjects} from '../src/home/scene/section-objects.js';
...
test('projected live text aligns with all four 3D corners at desktop and mobile sizes',()=>{
  ...
  const gallery=createShowcaseGallery({THREE,source:models.root,scene,integrated:true});
  for(const [vw,vh,w,h] of [[1440,900,560,430],[390,844,310,480],[320,640,240,380]]){
    ...
    for(let i=0;i<6;i++)for(const yaw of [-.22,0,.22]){
      ...
      const frame=sectionFrame(THREE,record,camera,{getBoundingClientRect:()=>({left:0,top:0,width:vw,height:vh})},{offsetWidth:w,offsetHeight:h,parentElement:{getBoundingClientRect:()=>parent}});
