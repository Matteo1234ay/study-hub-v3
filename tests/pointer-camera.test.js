...
test('mouse orbit changes perspective at rest without zoom or frame-rate dependence',()=>{
  ...
  assert.ok(a[0]>.1 && a[1]>0);assert.ok(Math.abs(Math.hypot(...a)-5)<1e-10);
...
test('pointer limits and neutral return keep the camera controlled',()=>{
  ...
  assert.ok(Math.abs(p[0])<.61 && Math.abs(p[1])<.33);
...
test('disassembly preserves solid opaque surfaces and separates them in depth',()=>{
  ...
  gallery.update(sampleShowcase(.55/6),0,{seconds:1,motion:1});
  ...
  assert.ok(Math.max(...depths)-Math.min(...depths)>.3);
