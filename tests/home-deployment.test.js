...
const motionToken = path => /(?:home-view|home-experience|showcase-renderer)\.js$/.test(path) ? "20260909-46" : /(?:study-room-renderer|home-v30-dematerialization)\.js$/.test(path) ? "20260906-34" : RELEASE_TOKEN;
...
test("the changed homepage chain uses the Safari-safe V30 token", async () => {
  ...
  assert.match(sources["index.html"], /src\/app\.js\?v=20260909-46/);
