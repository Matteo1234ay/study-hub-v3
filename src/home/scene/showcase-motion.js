import {sampleParticleTimeline} from './particle-timeline.js?v=20260909-46';
...
export function sampleShowcase(value, aspect = 1.6) {
  ...
  const opening = index === 5 ? 0 : windowProgress(local, .64, 1);
  const release = index === 5 ? 0 : windowProgress(local, .68, 1);
  ...
  const tunnel=Math.sin(Math.PI*release);
  const position = [x + orbit*.42, .22*orbit, distance*(1.22-.22*approach+.22*release)*(1-.46*tunnel)];
  const target = [x,0,0];
  return {
