...
export function renderMicroQuestion(question, { lessonId, chapterId, onReview = () => {}, onConsolidate = () => {}, onAnswered = () => {} } = {}) {
  ...
  function paint() {
    const options = attempt > 1 && validOptions(question.retryOptions) ? question.retryOptions : question.options;
    const fieldset = element("fieldset", {}, [element("legend", { text: attempt > 1 ? question.retryPrompt : question.prompt })]);
    options.forEach(option => { const input = element("input", { attrs: { type: "radio", name: `micro-${question.id}`, value: option.id } }); fieldset.append(element("label", { className: "micro-option" }, [input, element("span", { text: option.text })])); });
    ...
    submit.addEventListener("click", () => {
      ...
      feedback.textContent = result.feedback; feedback.className = `micro-feedback ${result.correct ? "is-correct" : "is-review"}`;
      if (selected) onAnswered(lessonId, chapterId, question.id);
      if (result.correct) { onConsolidate(lessonId, question); submit.disabled = true; }
      else if (selected) { onReview(lessonId, { ...question, chapterId }); if (result.canRetry) { submit.disabled = true; const retry = element("button", { className: "button quiet", text: "Prova con una domanda riformulata", attrs: { type: "button" } }); retry.addEventListener("click", () => { attempt = 2; paint(); }); feedback.after(retry); } }
    });
    root.replaceChildren(element("p", { className: "eyebrow", text: "Fermati e recupera" }), fieldset, submit, feedback, element("small", { text: "Puoi proseguire anche se sbagli: il concetto verrà riproposto nel ripasso." }));
  }
  paint(); return root;
}
