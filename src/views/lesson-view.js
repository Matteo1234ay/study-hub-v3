...
export async function renderLessonView({ lesson, activeChapterId = null, viewMode = "chapter" }) {
  ...
  const resolution = resolveLessonLocation(model, activeChapterId);
  const resolvedChapterId = resolution.chapterId;
  ...
  const store = createProgressStore(), studyStore = createStudyStore(), notesStore = createNotesStore(), reviewConceptsStore = createReviewConceptsStore();
  const assistant = createChatGptAdapter(), assistantDialog = createStudyDialog(); view.append(assistantDialog.node);
  studyStore.recordVisit({ type: "lesson", lessonId: lesson.id, title: lesson.title }); studyStore.setLastPosition(lesson.id, resolvedChapterId);
  const favorite = element("button", { className: `button quiet favorite-button${studyStore.getState().favorites.includes(lesson.id) ? " is-active" : ""}`, text: studyStore.getState().favorites.includes(lesson.id) ? "★ Nei preferiti" : "☆ Aggiungi ai preferiti", attrs: { type: "button" } });
  favorite.addEventListener("click", () => { const next = studyStore.toggleFavorite(lesson.id).favorites.includes(lesson.id); favorite.textContent = next ? "★ Nei preferiti" : "☆ Aggiungi ai preferiti"; favorite.classList.toggle("is-active", next); }); view.querySelector(".lesson-meta").append(favorite);
  ...
  const notesPanel = element("aside", { className: "lesson-notes-panel", attrs: { "aria-label": "Note della lezione" } });
  const notesTitle = element("h2", { text: "Note" }); const notesStatus = element("p", { className: "notes-status", text: "Salvate sul dispositivo" }); const notesList = element("div", { className: "structured-notes-list" });
  ...
  notesPanel.append(element("div", { className: "notes-panel-head" }, [notesTitle, closeNotes]), notesStatus, notesList, copyNotes); view.append(notesPanel);
  ...
  function paintLesson() {
    const saved = store.get(lesson.id), completed = new Set(saved.completed), bookmarks = new Set(studyStore.getState().bookmarks[lesson.id] ?? []); const percentage = calculateLessonProgress(model.chapters, completed); progressState.textContent = `${percentage}% completato`; progressBar.value = percentage;
    const lessonNode = renderLesson(model, { lessonId: lesson.id, assessmentEnabled: Boolean(lesson.assessmentUrl), activeChapterId: resolvedChapterId, viewMode, completedChapterIds: completed, bookmarkedChapterIds: bookmarks, noteForChapter: id => notesStore.get(lesson.id, id), onToggleChapter(id) { store.toggle(lesson.id, id); paintLesson(); }, onToggleBookmark(id) { studyStore.toggleBookmark(lesson.id, id); paintLesson(); }, onNote(id, text) { notesStore.set(lesson.id, id, text); }, onSectionNote(chapter, section) { const text = prompt(`Nota — ${section.title}`); if (text?.trim()) { notesStore.add(lesson.id, { chapterId: chapter.id, sectionId: section.id, text }); paintNotes(); notesPanel.classList.add("is-open"); } }, onDeepen(chapter) { assistantDialog.open(assistant.prepare(buildPublicChapterContext({ lesson, chapter }))); }, onReviewConcept(id, q) { reviewConceptsStore.markForReview(id, q); }, onConsolidateConcept(id, q) { reviewConceptsStore.clear(id, q.id); } }); body.replaceChildren(...[notice, lessonNode].filter(Boolean));
  }
  paintLesson();
  requestAnimationFrame(() => { const chapter = document.getElementById(resolvedChapterId); chapter?.scrollIntoView({ block: "start" }); chapter?.querySelector("h2")?.focus({ preventScroll: true }); if (resolution.sectionId) document.getElementById(resolution.sectionId)?.scrollIntoView({ block: "start" }); });
  return view;
}
